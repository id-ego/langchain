...
description: Playwright 브라우저 툴킷은 웹 브라우저를 자동화하여 동적 사이트와 상호작용할 수 있는 오픈소스 도구입니다.
---

# PlayWright 브라우저 툴킷

> [Playwright](https://github.com/microsoft/playwright)는 `Microsoft`에서 개발한 오픈 소스 자동화 도구로, 웹 브라우저를 프로그래밍 방식으로 제어하고 자동화할 수 있게 해줍니다. 이는 엔드 투 엔드 테스트, 스크래핑 및 `Chromium`, `Firefox`, `WebKit`과 같은 다양한 웹 브라우저에서 작업을 자동화하기 위해 설계되었습니다.

이 툴킷은 브라우저와 상호작용하는 데 사용됩니다. 다른 도구(`Requests` 도구와 같은)는 정적 사이트에 적합하지만, `PlayWright Browser` 툴킷은 에이전트가 웹을 탐색하고 동적으로 렌더링된 사이트와 상호작용할 수 있게 해줍니다.

`PlayWright Browser` 툴킷에 포함된 일부 도구는 다음과 같습니다:

- `NavigateTool` (navigate_browser) - URL로 탐색
- `NavigateBackTool` (previous_page) - 요소가 나타날 때까지 대기
- `ClickTool` (click_element) - 요소(선택자로 지정된) 클릭
- `ExtractTextTool` (extract_text) - 현재 웹 페이지에서 텍스트를 추출하기 위해 뷰티풀 수프 사용
- `ExtractHyperlinksTool` (extract_hyperlinks) - 현재 웹 페이지에서 하이퍼링크를 추출하기 위해 뷰티풀 수프 사용
- `GetElementsTool` (get_elements) - CSS 선택자로 요소 선택
...
```python
<!--IMPORTS:[{"imported": "PlayWrightBrowserToolkit", "source": "langchain_community.agent_toolkits", "docs": "https://api.python.langchain.com/en/latest/agent_toolkits/langchain_community.agent_toolkits.playwright.toolkit.PlayWrightBrowserToolkit.html", "title": "PlayWright Browser Toolkit"}]-->
from langchain_community.agent_toolkits import PlayWrightBrowserToolkit
```


비동기 함수로 컨텍스트를 생성하고 브라우저를 실행합니다:

```python
<!--IMPORTS:[{"imported": "create_async_playwright_browser", "source": "langchain_community.tools.playwright.utils", "docs": "https://api.python.langchain.com/en/latest/tools/langchain_community.tools.playwright.utils.create_async_playwright_browser.html", "title": "PlayWright Browser Toolkit"}]-->
...
`from_browser` 메서드를 사용하여 인스턴스화하는 것이 항상 권장됩니다.
...
여러 브라우저 도구는 `StructuredTool`이며, 이는 여러 인수를 기대합니다. 이러한 도구는 `STRUCTURED_CHAT_ZERO_SHOT_REACT_DESCRIPTION`보다 오래된 에이전트와 호환되지 않습니다.

```python
<!--IMPORTS:[{"imported": "AgentType", "source": "langchain.agents", "docs": "https://api.python.langchain.com/en/latest/agents/langchain.agents.agent_types.AgentType.html", "title": "PlayWright Browser Toolkit"}, {"imported": "initialize_agent", "source": "langchain.agents", "docs": "https://api.python.langchain.com/en/latest/agents/langchain.agents.initialize.initialize_agent.html", "title": "PlayWright Browser Toolkit"}, {"imported": "ChatAnthropic", "source": "langchain_anthropic", "docs": "https://api.python.langchain.com/en/latest/chat_models/langchain_anthropic.chat_models.ChatAnthropic.html", "title": "PlayWright Browser Toolkit"}]-->
...
{
"action": "Final Answer",
"action_input": "langchain.com 웹사이트의 주요 헤더는 다음과 같습니다:\n\n- LangChain\n- Products\n  - LangChain\n  - LangSmith\n  - LangGraph\n- Methods\n  - Retrieval\n  - Agents\n  - Evaluation\n- Resources\n  - Blog\n  - Case Studies\n  - Use Case Inspiration\n  - Experts\n  - Changelog\n- Docs\n  - LangChain Docs\n  - LangSmith Docs\n- Company\n  - About\n  - Careers\n  - Pricing\n- Get a demo\n- Sign up"
}
```[0m
