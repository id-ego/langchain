...
description: '`WebBaseLoader`를 사용하여 HTML 웹페이지의 모든 텍스트를 문서 형식으로 로드하는 방법을 설명합니다.'
---

# WebBaseLoader

이 문서는 `WebBaseLoader`를 사용하여 `HTML` 웹페이지의 모든 텍스트를 우리가 사용할 수 있는 문서 형식으로 로드하는 방법을 다룹니다. 웹페이지 로딩을 위한 더 많은 사용자 정의 논리가 필요하다면 `IMSDbLoader`, `AZLyricsLoader`, `CollegeConfidentialLoader`와 같은 자식 클래스 예제를 참고하세요.

웹사이트 크롤링, JS 차단 사이트 우회 및 데이터 정리에 대해 걱정하고 싶지 않다면 `FireCrawlLoader` 또는 더 빠른 옵션인 `SpiderLoader`를 사용하는 것을 고려하세요.

## 개요
### 통합 세부정보

- TODO: 테이블 기능을 채우세요.
- TODO: 관련이 없다면 JS 지원 링크를 제거하세요. 그렇지 않으면 링크가 정확한지 확인하세요.
- TODO: API 참조 링크가 정확한지 확인하세요.
...
`WebBaseLoader`를 사용하려면 먼저 `langchain-community` 파이썬 패키지를 설치해야 합니다.
...
```python
<!--IMPORTS:[{"imported": "WebBaseLoader", "source": "langchain_community.document_loaders", "docs": "https://api.python.langchain.com/en/latest/document_loaders/langchain_community.document_loaders.web_base.WebBaseLoader.html", "title": "WebBaseLoader"}]-->
...
가져오는 동안 SSL 인증 오류를 우회하려면 "verify" 옵션을 설정할 수 있습니다:
...
여러 URL을 동시에 크롤링하고 파싱하여 크롤링 프로세스를 가속화할 수 있습니다.

동시 요청에 대한 합리적인 제한이 있으며, 기본값은 초당 2입니다. 좋은 시민이 되는 것에 대해 걱정하지 않거나, 크롤링하는 서버를 제어하고 부하에 신경 쓰지 않는다면, `requests_per_second` 매개변수를 변경하여 최대 동시 요청 수를 늘릴 수 있습니다. 이로 인해 크롤링 프로세스는 빨라지지만, 서버가 당신을 차단할 수 있습니다. 조심하세요!
...
이 기능을 사용하는 예로 사이트맵 파일을 로드하는 방법에 대한 예제를 보려면 `SitemapLoader`를 참조하세요.
...
지연 로딩을 사용하여 메모리 요구 사항을 최소화하기 위해 한 번에 한 페이지만 로드할 수 있습니다.
...
때때로 IP 차단을 우회하기 위해 프록시를 사용해야 할 수도 있습니다. 로더(및 그 아래의 `requests`)에 프록시 사전을 전달하여 사용할 수 있습니다.
...
모든 `WebBaseLoader` 기능 및 구성에 대한 자세한 문서는 API 참조를 참조하세요: https://api.python.langchain.com/en/latest/document_loaders/langchain_community.document_loaders.web_base.WebBaseLoader.html
