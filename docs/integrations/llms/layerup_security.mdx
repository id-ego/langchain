---
description: Layerup Security는 LangChain LLM 호출을 안전하게 보호하는 통합 솔루션으로, 사용자와 LLM 간의 보안
  계층을 제공합니다.
---

# Layerup 보안

[Layerup 보안](https://uselayerup.com) 통합은 LangChain LLM, LLM 체인 또는 LLM 에이전트에 대한 호출을 안전하게 할 수 있도록 합니다. LLM 객체는 기존의 LLM 객체를 감싸며, 사용자와 LLM 사이에 안전한 레이어를 제공합니다.

Layerup 보안 객체는 LLM으로 설계되었지만, 실제로는 LLM이 아니며, 단순히 LLM을 감싸서 기본 LLM과 동일한 기능을 적용할 수 있게 합니다.

## 설정
먼저, Layerup [웹사이트](https://uselayerup.com)에서 Layerup 보안 계정을 만들어야 합니다.

다음으로, [대시보드](https://dashboard.uselayerup.com)를 통해 프로젝트를 생성하고 API 키를 복사합니다. API 키를 프로젝트의 환경에 넣는 것을 권장합니다.

Layerup 보안 SDK를 설치합니다:
```bash
pip install LayerupSecurity
```


그리고 LangChain 커뮤니티를 설치합니다:
```bash
pip install langchain-community
```


이제 Layerup 보안으로 LLM 호출을 보호할 준비가 되었습니다!
