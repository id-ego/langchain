...
> [AskNews](https://asknews.app)는 단일 자연어 쿼리를 사용하여 최신 글로벌 뉴스(또는 역사적 뉴스)를 모든 LLM에 주입합니다. 구체적으로, AskNews는 30만 개 이상의 기사를 매일 번역, 요약, 개체 추출 및 핫 및 콜드 벡터 데이터베이스에 인덱싱하여 풍부하게 하고 있습니다. AskNews는 이러한 벡터 데이터베이스를 저지연 엔드포인트에 제공합니다. AskNews에 쿼리를 입력하면, 가장 관련성 높은 풍부한 정보(예: 개체, 분류, 번역, 요약)를 포함하는 프롬프트 최적화 문자열이 반환됩니다. 이는 귀하가 자체 뉴스 RAG를 관리할 필요가 없으며, LLM에 뉴스 정보를 간결하게 전달하는 방법에 대해 걱정할 필요가 없음을 의미합니다. AskNews는 투명성에 전념하고 있으며, 이로 인해 우리의 보도는 수백 개 국가, 13개 언어 및 5만 개 출처에 걸쳐 모니터링되고 다양화됩니다. 우리의 출처 보도를 추적하고 싶다면 [투명성 대시보드](https://asknews.app/en/transparency)를 방문할 수 있습니다.
...
또한 [AskNews 콘솔](https://my.asknews.app)에서 생성할 수 있는 AskNews API 자격 증명을 설정해야 합니다.
...
```python
<!--IMPORTS:[{"imported": "AskNewsRetriever", "source": "langchain_community.retrievers", "docs": "https://api.python.langchain.com/en/latest/retrievers/langchain_community.retrievers.asknews.AskNewsRetriever.html", "title": "AskNews"}]-->
...
```python
<!--IMPORTS:[{"imported": "StrOutputParser", "source": "langchain_core.output_parsers", "docs": "https://api.python.langchain.com/en/latest/output_parsers/langchain_core.output_parsers.string.StrOutputParser.html", "title": "AskNews"}, {"imported": "ChatPromptTemplate", "source": "langchain_core.prompts", "docs": "https://api.python.langchain.com/en/latest/prompts/langchain_core.prompts.chat.ChatPromptTemplate.html", "title": "AskNews"}, {"imported": "RunnablePassthrough", "source": "langchain_core.runnables", "docs": "https://api.python.langchain.com/en/latest/runnables/langchain_core.runnables.passthrough.RunnablePassthrough.html", "title": "AskNews"}, {"imported": "ChatOpenAI", "source": "langchain_openai", "docs": "https://api.python.langchain.com/en/latest/chat_models/langchain_openai.chat_models.base.ChatOpenAI.html", "title": "AskNews"}]-->
