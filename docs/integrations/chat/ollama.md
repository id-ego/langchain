...
description: Ollama는 Llama 2와 같은 오픈 소스 대형 언어 모델을 로컬에서 실행할 수 있게 해주는 플랫폼입니다. 모델 관리가
  용이합니다.
...
Ollama는 모델 가중치, 구성 및 데이터를 Modelfile로 정의된 단일 패키지로 묶습니다.

GPU 사용을 포함한 설정 및 구성 세부정보를 최적화합니다.

지원되는 모델 및 모델 변형의 전체 목록은 [Ollama 모델 라이브러리](https://github.com/jmorganca/ollama#model-library)를 참조하십시오.

## 개요
### 통합 세부정보

| 클래스 | 패키지 | 로컬 | 직렬화 가능 | [JS 지원](https://js.langchain.com/v0.2/docs/integrations/chat/ollama) | 패키지 다운로드 | 패키지 최신 |
| :--- | :--- | :---: | :---: |  :---: | :---: | :---: |
...
* [다운로드](https://ollama.ai/download)하고 Ollama를 지원되는 플랫폼(Windows Subsystem for Linux 포함)에 설치합니다.
* `ollama pull <모델 이름>`을 통해 사용 가능한 LLM 모델을 가져옵니다.
  * [모델 라이브러리](https://ollama.ai/library)를 통해 사용 가능한 모델 목록을 확인합니다.
...
* 관심 있는 모델의 정확한 버전을 지정하려면 `ollama pull vicuna:13b-v1.5-16k-q4_0`와 같이 입력합니다. (이 경우 [Vicuna](https://ollama.ai/library/vicuna/tags) 모델의 다양한 태그를 확인하십시오.)
* 모든 다운로드된 모델을 보려면 `ollama list`를 사용합니다.
* 명령줄에서 모델과 직접 대화하려면 `ollama run <모델 이름>`을 사용합니다.
* 더 많은 명령어는 [Ollama 문서](https://github.com/jmorganca/ollama)를 참조하십시오. 터미널에서 `ollama help`를 실행하여 사용 가능한 명령어를 확인할 수도 있습니다.
...
```python
<!--IMPORTS:[{"imported": "ChatOllama", "source": "langchain_ollama", "docs": "https://api.python.langchain.com/en/latest/chat_models/langchain_ollama.chat_models.ChatOllama.html", "title": "ChatOllama"}]-->
...
```python
<!--IMPORTS:[{"imported": "AIMessage", "source": "langchain_core.messages", "docs": "https://api.python.langchain.com/en/latest/messages/langchain_core.messages.ai.AIMessage.html", "title": "ChatOllama"}]-->
...
```python
<!--IMPORTS:[{"imported": "ChatPromptTemplate", "source": "langchain_core.prompts", "docs": "https://api.python.langchain.com/en/latest/prompts/langchain_core.prompts.chat.ChatPromptTemplate.html", "title": "ChatOllama"}]-->
...
우리는 [도구 호출](https://blog.langchain.dev/improving-core-tool-interfaces-and-docs-in-langchain/)을 사용할 수 있습니다 [도구 사용을 위해 미세 조정된 LLM](https://ollama.com/library/llama3-groq-tool-use):
...
우리는 일반 Python 함수를 도구로 직접 전달할 수 있습니다.
...
우리는 LangSmith 추적을 살펴보아 도구 호출이 수행되었음을 확인합니다:

https://smith.langchain.com/public/4169348a-d6be-45df-a7cf-032f6baa4697/r

특히, 추적은 도구 스키마가 어떻게 채워졌는지를 보여줍니다.

## 다중 모드

Ollama는 [bakllava](https://ollama.com/library/bakllava) 및 [llava](https://ollama.com/library/llava)와 같은 다중 모드 LLM을 지원합니다.

    ollama pull bakllava

다중 모드를 지원하기 위해 최신 버전의 Ollama로 업데이트해야 합니다.
...
ChatOllama의 모든 기능 및 구성에 대한 자세한 문서는 API 참조를 참조하십시오: https://api.python.langchain.com/en/latest/chat_models/langchain_ollama.chat_models.ChatOllama.html
